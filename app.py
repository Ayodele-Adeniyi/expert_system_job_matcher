...
if "educations" not in st.session_state:
    st.session_state.educations = [{"highest_degree": HIGHEST_DEGREE_OPTIONS[3], "degree_field": "Computer Science"}]
        ...
        for i, edu in enumerate(st.session_state.educations):
            with st.container(border=True):
                ...
                edu["highest_degree"] = st.selectbox(
                    ...
                    HIGHEST_DEGREE_OPTIONS,
                    key=f"deg_level_{i}",
                    index=HIGHEST_DEGREE_OPTIONS.index(edu["highest_degree"]) if edu["highest_degree"] in HIGHEST_DEGREE_OPTIONS else 3,
                )
                edu["degree_field"] = st.selectbox(
                    "Field of Study",
                    DEGREE_FIELD_OPTIONS,
                    key=f"deg_field_{i}",
                    index=DEGREE_FIELD_OPTIONS.index(edu["degree_field"]) if edu["degree_field"] in DEGREE_FIELD_OPTIONS else 0,
                )
        ...
        with b1:
            if st.button("Add Another Degree", use_container_width=True):
                if len(st.session_state.educations) < 5:
                    st.session_state.educations.append({"highest_degree": HIGHEST_DEGREE_OPTIONS[3], "degree_field": "Other"})
                    st.rerun()
                else:
...
with right_col:
    ...
    if evaluate_button:
        if not first_name.strip() or not last_name.strip():
            ...
            facts = {"first_name": first_name.strip(), "last_name": last_name.strip()}
    ...
    if st.session_state.results is None:
        ...
        with m2:
            st.metric("Avg Required Match", f"{sum(r.required_match_pct for r in results) / len(results):.1f}%")
        with m3:
            st.metric("Avg Total Match", f"{sum(r.total_match_pct for r in results) / len(results):.1f}%")
        ...
        fig = go.Figure()
        fig.add_trace(go.Bar(name="Required", x=df["Position"], y=df["Required Match"], text=df["Required Match"].round(1), textposition="outside"))
        fig.add_trace(go.Bar(name="Desired", x=df["Position"], y=df["Desired Match"], text=df["Desired Match"].round(1), textposition="outside"))
        fig.update_layout(barmode="group", height=380, xaxis_title="Position", yaxis_title="Match Percent")
                    ...
                    if not r.desired_met and not r.desired_missing:
                        ...
                        if r.desired_missing:
                            ...
                            for des in r.desired_missing:
                                st.write(f"⭐ {des.message}")
                                st.caption(f"Expected: {des.expected} | Actual: {des.actual if des.actual is not None else 'Not provided'}")
                        st.progress(r.desired_match_pct / 100.0, text=f"Desired Match: {r.desired_match_pct:.1f}%")
